...
router.post('/',(req,res)=>{
    let {name}=req.body
    if(!name){
        req.logger.log("error", `Complete al menos la propiedad name`)
    ...
    if(!valido){
        req.logger.log("error", `Ha ingresado propiedades invalidas. \nProps. ingresadas: ${JSON.stringify(req.body, null, 5)}`)
        res.setHeader('Content-Type','application/json');
