...
router.get("/exit", (req, res)=>{
    ...
    res.status(200).json({payload:"saliendo..."});
    process.exit()
})
...
router.get('/op1',(req,res)=>{
    ...
    if(valor1<30){
        console.log("BAJA PROCESO... ************", valor1, process.pid)
        process.exit()
    }

    console.log(valor1, valor2, resultado, `pid resolviendo: ${process.pid}`)
...
router.get('/op2',(req,res)=>{
    ...
    console.log(contador, resultado, `pid resolviendo: ${process.pid}`)
    console.timeEnd("Demora operación:")
